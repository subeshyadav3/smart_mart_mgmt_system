...
export const createStaff = async (req, res, next) => {
  try {
    const { fullName, email, password, phoneNumber, role = "STAFF", isActive = true } = req.body;
    ...
    res.status(201).json({ success: true, data: sanitize(staff) });
  } catch (err) {
    next(err);
  }
};
...
export const getAllStaffs = async (req, res, next) => {
  try {
    ...
    res.json({ success: true, data: staffs.map(sanitize) });
  } catch (err) {
    next(err);
  }
};
...
export const updateStaffRole = async (req, res, next) => {
  try {
    ...
    res.json({ success: true, data: sanitize(staff) });
  } catch (err) {
    next(err);
  }
};
